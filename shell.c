    ...
    if(!builtin_cmd(argv)){
        ...
        for(int i=9; i>=0; i--){
            ...
            while(strcmp(curr_cmd, "NOMORE")){
                pipe(fd);
                printf("Created a pipe in:%d, out:%d",fd[0],fd[1]);
                fflush(stdout);
                ...
                if ((pid = fork ()) == 0)
                {
                    /** If this is not the first command being executed
                    then change the input to pipe input */
                    //printf("num_cmd %d i %d size %d\n", num_commands, i, queues[i].size);
                    //printf("arv[0] : %s]\n", argv[0]); 
                    //dup2(1, 2);
                    if (num_commands!=0)
                    {
                        //printf("Not first so switching input in:%d \n", in);
                        dup2(in, 0);
                        printf("Switched input for %s with %d\n",curr_cmd,in);
                        fflush(stdout);
                        close(in);
                    }else{
                        close(in);
                    ...
                    /** If this is not the last command being executed
                    then change the output to the pipe output */
                    if (!(commands_rem<=1))
                    {
                        //printf("Not last so switching output\n");
                        printf("Switching output for %s with %d\n",curr_cmd,fd[1]);
                        fflush(stdout);
                        dup2(fd[1], 1);
                    }
